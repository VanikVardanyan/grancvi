...
## TMA-задача. Загрузка аватара мастера на странице профиля

**Контекст.** Backend готов: `POST /v1/master/me/avatar` (multipart, поле `file`, лимит 10 MB, принимает JPEG/PNG/WebP/HEIC; на выходе 256×256 JPEG, сохраняется атомарно) и `DELETE /v1/master/me/avatar`. Оба требуют `X-Telegram-Init-Data`. Сразу после успеха `avatar_url` возвращается в ответе и в `GET /v1/me` (а также в `/v1/public/masters/{slug}` и `/v1/public/salons/{slug}` для клиентской стороны).

**Что сделать на TMA:**
- В мастер-профиле — секция «Аватар» (рядом с именем/специальностью).
- Если `avatar_url is None` — большая круглая плашка-плейсхолдер с CTA «Загрузить фото». При тапе открывается нативный picker (на iPhone HEIC поддерживается на бэке).
- Если есть — показываем `<API_ORIGIN><avatar_url>` (querystring `?v=` обеспечивает cache-bust при замене), две кнопки: «Заменить» и «Удалить».
- На замену — повторный picker, тот же `POST` (бэк перезапишет). На удаление — `DELETE`, после успеха показываем плашку без фото.
- Frontend-валидация ДО запроса: тип `image/*`, размер ≤ 10 MB. Если не прошло — toast «Файл слишком большой» / «Это не картинка», не отправляем запрос.
- Обработка ошибок API:
  - `413 avatar_too_large` → toast «Файл слишком большой, до 10 MB»
  - `400 bad_image` → toast «Не удалось прочитать картинку»
  - `401` → перезагрузить init-data (общая логика TMA)
  - сеть/5xx → стандартный retry-toast TMA
- Прогресс загрузки: spinner или прогресс-бар, кнопки выключены до завершения.

**Критерии готовности:**
- Мастер видит свой аватар в профиле TMA и в публичной странице `/master_<slug>` / `/salon_<slug>` (после `?v=` URL обновляется без жёсткой перезагрузки).
- iPhone HEIC из камеры/галереи доходит до бэка и сохраняется как JPEG (бэк уже умеет — нужна только корректная отправка multipart с правильным `Content-Type` от браузера, без кастомного перекодирования).
- Удаление сразу убирает фото и из TMA, и из публичной страницы.

**Файлы (примерно, frontend-репозиторий):** компонент `MasterProfile/AvatarSection.tsx`, хук вокруг fetch (`POST/DELETE /v1/master/me/avatar`), стили под круглую плашку.

**Backend-зависимости:** уже на проде после мерджа этой ветки. Бэкенд тесты — `tests/test_api_master_avatar.py`, `tests/test_api_public_avatar.py`.
